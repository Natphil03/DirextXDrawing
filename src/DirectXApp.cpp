...
{
	_worldTransformation = Matrix::CreateTranslation(Vector3(0.0f, 0.0f, 0.0f)) * Matrix::CreateRotationY(_rotationAngle * XM_PI / 180.0f);
	_worldTransformation2 = Matrix::CreateTranslation(Vector3(4.0f, -1.0f, 0.0f)) * Matrix::CreateRotationY(_rotationAngle * XM_PI / 180.0f);
...
void DirectXApp::Render()
{
	...
	_deviceContext->DrawIndexed(ARRAYSIZE(indicesCube), 0, 0);

	

	//Draw 2nd cube

	completeTransformation = _worldTransformation2 * _viewTransformation * _projectionTransformation;
	constantBuffer.WorldViewProjection = completeTransformation;

	_deviceContext->IASetVertexBuffers(0, 1, _vertexBuffer2.GetAddressOf(), &stride, &offset);
	_deviceContext->IASetIndexBuffer(_indexBuffer2.Get(), DXGI_FORMAT_R32_UINT, 0);

	_deviceContext->UpdateSubresource(_constantBuffer.Get(), 0, 0, &constantBuffer, 0, 0);
	_deviceContext->DrawIndexed(ARRAYSIZE(indicesPyramid), 0, 0);
...
void DirectXApp::BuildGeometryBuffers()
{
	...
	vertexBufferDescriptor.ByteWidth = sizeof(Vertex) * ARRAYSIZE(verticesCube);
	vertexBufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vertexBufferDescriptor.CPUAccessFlags = 0;
	vertexBufferDescriptor.MiscFlags = 0;
	vertexBufferDescriptor.StructureByteStride = 0;

	D3D11_BUFFER_DESC vertexBufferDescriptor2 = { 0 };
	vertexBufferDescriptor2.Usage = D3D11_USAGE_IMMUTABLE;
	vertexBufferDescriptor2.ByteWidth = sizeof(Vertex) * ARRAYSIZE(verticesPyramid);
	vertexBufferDescriptor2.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vertexBufferDescriptor2.CPUAccessFlags = 0;
	vertexBufferDescriptor2.MiscFlags = 0;
	vertexBufferDescriptor2.StructureByteStride = 0;

	// Now set up a structure that tells DirectX where to get the
	// data for the vertices from
	D3D11_SUBRESOURCE_DATA vertexInitialisationData = { 0 };
	vertexInitialisationData.pSysMem = &verticesCube;

	D3D11_SUBRESOURCE_DATA vertexInitialisationData2 = { 0 };
	vertexInitialisationData2.pSysMem = &verticesPyramid;

	// and create the vertex buffer
	ThrowIfFailed(_device->CreateBuffer(&vertexBufferDescriptor, &vertexInitialisationData, _vertexBuffer.GetAddressOf()));
	ThrowIfFailed(_device->CreateBuffer(&vertexBufferDescriptor2, &vertexInitialisationData2, _vertexBuffer2.GetAddressOf()));

	// Setup the structure that specifies how big the index 
	// buffer should be
	D3D11_BUFFER_DESC indexBufferDescriptor = { 0 };
	indexBufferDescriptor.Usage = D3D11_USAGE_IMMUTABLE;
	indexBufferDescriptor.ByteWidth = sizeof(UINT) * ARRAYSIZE(indicesCube);
	indexBufferDescriptor.BindFlags = D3D11_BIND_INDEX_BUFFER;
	indexBufferDescriptor.CPUAccessFlags = 0;
	indexBufferDescriptor.MiscFlags = 0;
	indexBufferDescriptor.StructureByteStride = 0;

	D3D11_BUFFER_DESC indexBufferDescriptor2 = { 0 };
	indexBufferDescriptor2.Usage = D3D11_USAGE_IMMUTABLE;
	indexBufferDescriptor2.ByteWidth = sizeof(UINT) * ARRAYSIZE(indicesPyramid);
	indexBufferDescriptor2.BindFlags = D3D11_BIND_INDEX_BUFFER;
	indexBufferDescriptor2.CPUAccessFlags = 0;
	indexBufferDescriptor2.MiscFlags = 0;
	indexBufferDescriptor2.StructureByteStride = 0;

	// Now set up a structure that tells DirectX where to get the
	// data for the indices from
	D3D11_SUBRESOURCE_DATA indexInitialisationData;
	indexInitialisationData.pSysMem = &indicesCube;
	D3D11_SUBRESOURCE_DATA indexInitialisationData2;
	indexInitialisationData2.pSysMem = &indicesPyramid;

	 //and create the index buffer


	ThrowIfFailed(_device->CreateBuffer(&indexBufferDescriptor, &indexInitialisationData, _indexBuffer.GetAddressOf()));
	ThrowIfFailed(_device->CreateBuffer(&indexBufferDescriptor2, &indexInitialisationData2, _indexBuffer2.GetAddressOf()));
}
